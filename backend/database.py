...
def delete_menu_item(item_id, name, category, price):
    """Delete 1 item from the menu table"""
    connection = sqlite3.connect(DB_NAME)
    cursor = connection.cursor()
    cursor.execute("DELETE FROM menu WHERE item_id=? AND name=? AND category=? AND price=?", (item_id, name, category, price))
    connection.commit()
    connection.close()
    print(f"Menu item '{name}' deleted from Menu!")

def update_menu_item(new_name, new_category, new_price):
    ...
    connection.commit()
    connection.close()
    print(f"Menu item '{new_name}' updated in Menu!")
